  ...
  const content = {
    en: {
      ...
      services: {
        ...
        items: [
          ...
          },
          {
            ...
            list: ["Branding", "Video production", "Social media designs", "Motion Graphics"]
          },
          {
            title: "Marketing & Campaign Execution",
            list: ["Integrated marketing campaigns", "Public awareness campaigns"]
          }
        ]
      },
      partners: {
      ...
      work: {
        ...
        projects: [
          {
            title: "Ministry of National Education - Madrasati Awlan campaign - 2026",
          ...
          {
            title: "Sudanese Mineral Resources Company Limited - Documentary Film – Corporate Social Responsibility 2024",
          ...
          {
            title: "King Salman Humanitarian Aid & Relief Centre – Food Security Support Project 2024",
          ...
          {
            title: "Saif Al-Dawla Food Industries – Social Media Management & Advertising Campaigns",
      ...
      values: {
        title: "Core Values",
        list: ["Integrity - Accountability", "Cultural awareness - Professionalism"]
      },
      contact: {
        title: "Contact Us",
        ...
        items: [
          {
            ...
            list: ["استراتيجية وسائل التواصل الاجتماعي وإدارتها", "إنشاء المحتوى"]
          },
          {
            title: "الإبداع والتصميم",
            list: ["العلامات التجارية", "إنتاج الفيديو", "تصاميم وسائل التواصل", "الرسوم المتحركة"]
          },
          {
    ...
    <div className={`app ${isArabic ? 'rtl' : 'ltr'}`}>
      <nav className="navbar" ref={navRef}>
        ...
        <ul className={`nav-links ${menuOpen ? 'open' : ''}`}>
          <li><a href="#about" onClick={() => setMenuOpen(false)}>{currentContent.nav.about}</a></li>
          <li><a href="#services" onClick={() => setMenuOpen(false)}>{currentContent.nav.services}</a></li>
          <li><a href="#partners" onClick={() => setMenuOpen(false)}>{currentContent.nav.partners}</a></li>
          <li><a href="#work" onClick={() => setMenuOpen(false)}>{currentContent.nav.work}</a></li>
          <li><a href="#contact" onClick={() => setMenuOpen(false)}>{currentContent.nav.contact}</a></li>
      ...
      <section className="section section-hero">
        <div className="container hero-content">
          {/* <div className="logo-badge bg-primary">SC</div> */}
          <h1 className="hero-title text-primary">{currentContent.hero.title}</h1>
          <p className="hero-text">{currentContent.hero.subtitle}</p>
          {/* <p className="hero-year">{currentContent.hero.year}</p> */}
        </div>
      </section>
      ...
      <section className="section">
        <div className="container">
          <h2 className="section-title centered">{currentContent.whyChoose.title}</h2>
          <div className="feature-grid">
            {currentContent.whyChoose.features.map((feature, index) => (
              <article key={index} className="feature-card">
                <div className="icon-circle">
                  <i className={`fas fa-${['brain', 'chart-line', 'users', 'cogs', 'rocket'][index]}`}></i>
                </div>
                <h3>{feature}</h3>
              </article>
            ))}
      ...
      <section className="section section-alt" id="services">
        <div className="container">
          <h2 className="section-title centered">{currentContent.services.title}</h2>
          <div className="service-grid">
            {currentContent.services.items.map((service, index) => (
              <article key={index} className="service-card">
                <div className="icon-circle">
                  <i className={`fas fa-${['share-alt', 'palette', 'bullhorn'][index]}`}></i>
                </div>
                <h3>{service.title}</h3>
                <ul>
      ...
      <section className="section" id="partners">
        <div className="container">
          <h2 className="section-title centered">{currentContent.partners.title}</h2>
          <div className="partner-grid">
            {currentContent.partners.list.map((partner, index) => (
              <article key={index} className="partner-card">{partner}</article>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="section section-alt" id="work">
        <div className="container">
          <h2 className="section-title centered">{currentContent.work.title}</h2>
              ...
              <article key={index} className="project-card">
                ...
                <div className="project-meta">
                  {project.meta.map((meta, i) => (
                    <span key={i}>
                      <i className={`fas fa-${['play', 'film', 'play', 'play', 'play', 'play', 'play', 'play'][index]}`}></i>
                      {meta}
                    </span>
                  ))}
                </div>
      ...
      <section className="section">
        <div className="container">
          <h2 className="section-title centered">{currentContent.process.title}</h2>
          <div className="process-grid">
            {currentContent.process.steps.map((step, index) => (
              <article key={index} className="process-card">
                <div className="icon-circle">
                  <i className={`fas fa-${['search', 'lightbulb', 'pencil-alt', 'tools', 'check-circle', 'rocket'][index]}`}></i>
                </div>
                <h3>{step}</h3>
              </article>
            ))}
      ...
      <section className="section" id="contact">
        <div className="container">
          <h2 className="section-title centered">{currentContent.contact.title}</h2>
          <div className="contact-grid">
            <article className="contact-panel">
              <h3>{currentContent.contact.getInTouch}</h3>
              <p><i className="fas fa-phone"></i> +249 123 456 789</p>
              <p><i className="fas fa-envelope"></i> info@sharikmedia.com</p>
              <p><i className="fas fa-map-marker-alt"></i> Khartoum, Sudan</p>
            </article>
            <article className="contact-panel">
